...
class Batchifier():
    ...
    def f(self, recv, send):
        ...
        from TFIDFWordEmbeddingAbstractsModel import TFIDFWordEmbeddingAbstractsModel
        model = TFIDFWordEmbeddingAbstractsModel()
        model._load_model()
...
if __name__ == '__main__':
    ...
    filename = "abstracts.tfidf.wordembedding.train.pkl"
    ...
    model = TFIDFWordEmbeddingAbstractsModel(embedding_model, pretrained = False)
    model.train(d_train.data, embedding_model)
    
    ### load test data if it is already pickled, otherwise create it from scratch
    
    filename = "abstracts.tfidf.wordembedding.test.pkl"
